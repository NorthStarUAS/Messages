...
public:
    ...
    void msg2props(PropertyNode node) {
        ...
        for (int _i=0; _i<max_raw_sats; _i++) node.setUInt("svid", _i, svid[_i]);
        for (int _i=0; _i<max_raw_sats; _i++) node.setDouble("pseudorange", _i, pseudorange[_i]);
        for (int _i=0; _i<max_raw_sats; _i++) node.setDouble("doppler", _i, doppler[_i]);
    }
...
public:
    ...
    void msg2props(PropertyNode node) {
        node.setUInt("index", index);
        node.setDouble("timestamp_sec", timestamp_sec);
        for (int _i=0; _i<8; _i++) node.setDouble("channel", _i, channel[_i]);
        node.setUInt("status", status);
    }
